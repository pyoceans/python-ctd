...
def proc_ctd(fname, below_water=True):
    ...
    cast = DataFrame.from_cnv(fname, below_water=below_water).split()[0]
    ...
    if False:
        ...
        if pmax >= 500.0:
            ...
            window_len = 5
            kw = {"window_len": window_len, "window": "hanning"}
...
def test_header_parse():
    lon, lat = "-42", "42"
    xbt1 = DataFrame.from_edf(
        data_path.joinpath("C3_00005.edf"), lon=lon, lat=lat
    )
    assert (xbt1.longitude, xbt1.latitude) == (float(lon), float(lat))

    with pytest.raises(ValueError):
        DataFrame.from_edf(
            data_path.joinpath("C3_00005.edf"), lon=None, lat=None
        )
